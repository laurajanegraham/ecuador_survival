...
recap.rec <- filter(banding.dat.clean, Band.Size == "RE")

# get only records recorded as first banding
banding.rec <- filter(banding.dat.clean, Band.Size != "RE")
...
no.first.band <- filter(recap.rec, !(Band.Number %in% banding.rec$Band.Number)) %>%
        group_by(Specie.Name, Band.Number) %>%
...
dup.bands <- filter(encounter.history, Band.Number %in% dupes)
