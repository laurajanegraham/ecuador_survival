...
# code for multinomial formulation with constant parameters --------------------
sink("~/.wine/drive_c/temp/Rtmp/cjs-mnl-fixed.bug")
cat("
model {
    # Priors and constraints
    for (t in 1:(n.occasions-1)){
    phi[t] <- mean.phi          # Priors for survival
...
# code for multinomial formulation with fixed group effects (2 groups only) ----
# some species are not present in the native habitat, needed to rerun the
# habitat analysis for these species with only 2 groups
sink("~/.wine/drive_c/temp/Rtmp/cjs-mnl-habitat-2groups.bug")
cat("
    model {
    # Priors and constraints
    for (t in 1:(n.occasions-1)){
...
sink("~/.wine/drive_c/temp/Rtmp/cjs-mnl-TSM.bug")
cat("
model {
    # Priors and constraints
    for (t in 1:(n.occasions-1)){
    phi.TSM1[t] <- mean.phitsm1
    phi.TSM2[t] <- mean.phitsm2
    p[t] <- mean.p
    }
    mean.phitsm1 ~ dunif(0, 1)          # Prior for mean juv. survival
    mean.phitsm2 ~ dunif(0, 1)           # Prior for mean ad. survival
    mean.p ~ dunif(0, 1)               # Prior for mean recapture
    # Define the multinomial likelihood
    for (t in 1:(n.occasions-1)){
    ...
    # Calculate the number of birds released each year
    for (t in 1:(n.occasions-1)){
    r.TSM1[t] <- sum(marr.TSM1[t,])
    r.TSM2[t] <- sum(marr.TSM2[t,])
    }
    # Define the cell probabilities of the m-arrays
    # Main diagonal
    for (t in 1:(n.occasions-1)){
