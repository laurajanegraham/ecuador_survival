...
colnames(survival.res)[3] <- "sigma2"
select(survival.res, -Species)
```

- `mean.phi` - overall survival probability (across all sampling periods)
- `mean.p` - recapture probability
- `sigma2` - variance of survival probability (across all sampling periods)
...
- Possibility to estimate variance of survival probability means can incorporate a temporal covariate
- Needs more work on model selection / goodness of fit testing
- Run for more species and models