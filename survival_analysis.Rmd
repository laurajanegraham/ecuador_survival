...
dat <- ldply(data_input, fread) %>% 
  ...
  mutate(habitat = ifelse(Location %in% c("LLAV", "SANA"), "Shrubs", ifelse(Location == "MASE", "Forest", "Grasslands")),
...
hab_comp <- filter(sims_narrow, param_name == "mean.phi") %>% 
  ...
  mutate(Habitat = factor(h, levels=c(2, 1, 3), 
                          labels=c("Forest", "Grasslands", "Shrubs"))) %>% 
...
hab_comp_plot <- ggplot(hab_comp, aes(x=value, fill=param_name)) + 
  geom_density(alpha=0.5, position="identity") + 
  scale_fill_viridis_d(name = "", labels = c(expression(phi[Grasslands] - phi[Forest]), 
                                  expression(phi[Grasslands] - phi[Shrubs]), 
                                  expression(phi[Forest] - phi[Shrubs]))) + 
  geom_vline(data = hab_summary, aes(xintercept = value, linetype = param_name)) + 
  scale_linetype_discrete(name = "", labels = c(expression(phi[Grasslands] - phi[Forest]), 
                                  expression(phi[Grasslands] - phi[Shrubs]), 
                                  expression(phi[Forest] - phi[Shrubs]))) + 
...
survival_hab_comp <- filter(summ, param_name == "phi") %>% 
  ...
  mutate(species_no = species %>% as.integer,
         habitat = factor(habitat, levels=c(2,1,3), labels=c("Forest", "Grasslands", "Shrubs"))) %>% 
  select(-species) %>% 
  inner_join(arrange(species_info, species) %>% 
               mutate(species_no = species %>% as.factor %>% as.numeric)) %>% 
